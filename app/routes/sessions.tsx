...
type DemoState = 'idle' | 'evaluating' | 'result';
  ...
  return (
    <div className="mt-3">
      <div className="flex justify-between items-center mb-1">
        <span className="text-[11px] font-semibold text-slate-500 uppercase tracking-wide">Risk Score</span>
        <span className={`text-[12px] font-bold ${
          score >= 80 ? 'text-red-600' : score >= 40 ? 'text-amber-600' : 'text-emerald-600'
        }`}>{score}/100</span>
      </div>
  ...
  return (
    <div className="flex items-center justify-between py-2 border-b border-slate-100 last:border-0">
      <div className="flex items-center gap-2 min-w-0">
        <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded border uppercase tracking-wide flex-shrink-0 ${SEVERITY_COLORS[rule.severity] || SEVERITY_COLORS.low}`}>
          {rule.severity}
        </span>
        <span className="text-[13px] text-slate-700 truncate">{rule.name}</span>
      </div>
      <span className="text-[12px] font-semibold text-slate-500 flex-shrink-0 ml-2">+{rule.score_impact}pts</span>
    </div>
  );
}
...
function ResultCard({
  ...
  const { status, riskScore, triggeredRules, approvalId, responseText, tokensUsed, sessionId } = result;
      ...
      <div className="bg-white border border-slate-200 rounded-xl border-l-4 border-l-emerald-500 shadow-sm overflow-hidden">
        <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-slate-800 text-[14px]">Session Completed — Compliance Check Passed</span>
          </div>
        ...
        <div className="px-5 py-4">
          ...
          <div className="mt-3 flex items-center gap-4 text-[11px] text-slate-400 border-t border-slate-100 pt-3">
            ...
            <span>{triggeredRules?.length ?? 0} violation{(triggeredRules?.length ?? 0) !== 1 ? 's' : ''}</span>
            <span className="text-slate-200">·</span>
      ...
      <div className="bg-white border border-slate-200 rounded-xl border-l-4 border-l-amber-400 shadow-sm overflow-hidden">
        <div className="px-5 py-4 border-b border-amber-50 bg-amber-50/40 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-slate-800 text-[14px]">Session Frozen — Compliance Review Required</span>
          </div>
        ...
        <div className="px-5 py-4">
          <p className="text-[13px] text-slate-600 mb-4">
            The Circuit Breaker detected a policy boundary breach. The AI response has been withheld pending human review.
          </p>
          <div className="mb-4">
            <p className="text-[11px] font-semibold text-slate-500 uppercase tracking-wide mb-2">Triggered Rules</p>
            <div className="rounded-lg border border-amber-100 bg-amber-50/30 px-3 divide-y divide-amber-100">
              {triggeredRules.map(rule => <TriggeredRuleRow key={rule.id} rule={rule} />)}
            </div>
          </div>
          {approvalId && (
            <div className="bg-slate-50 border border-slate-200 rounded-lg px-4 py-3 mb-4 text-[12px]">
              ...
              <p className="text-slate-500 mt-1">A reviewer has been notified. The AI response is withheld until approved.</p>
            </div>
          )}
      ...
      <div className="bg-white border border-slate-200 rounded-xl border-l-4 border-l-red-500 shadow-sm overflow-hidden">
        <div className="px-5 py-4 border-b border-red-50 bg-red-50/40 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-slate-800 text-[14px]">Session Blocked — Compliance Violation</span>
          </div>
        ...
        <div className="px-5 py-4">
          <p className="text-[13px] text-slate-600 mb-4">
            Blocked at pre-flight. <strong className="text-slate-800">No AI call was made.</strong> The prompt triggered one or more critical compliance rules.
          </p>
          <div className="mb-4">
            <p className="text-[11px] font-semibold text-slate-500 uppercase tracking-wide mb-2">Triggered Rules</p>
          ...
          <p className="text-[11px] text-red-700 mt-1.5 font-medium">Risk Score: {riskScore}/100 — Threshold exceeded</p>
...
export default function Sessions() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!promptText.trim() || demoState === 'evaluating') return;
    if (!operator) return;
    ...
    try {
      // Step 1: Submit session (compliance evaluation)
      ...
      // Step 2: If pending — call AI
      await new Promise(r => setTimeout(r, 400)); // let final step animate
      const aiRes = await apiFetch('/api/ai/generate', {
        method: 'POST',
        body: JSON.stringify({
      ...
      <div className="p-6 max-w-[1400px] mx-auto">
        ...
        <div className="mb-6">
          <div className="flex items-center gap-3 mb-1">
            ...
            <span className="text-[11px] font-semibold bg-blue-50 text-blue-600 px-2 py-0.5 rounded-full border border-blue-200 uppercase tracking-wide">Circuit Breaker</span>
          </div>
          <p className="text-[13px] text-slate-500 ml-10">Submit prompts through Axiom's compliance engine before AI processing</p>
        </div>
          ...
          <div className="flex flex-col gap-4">
            <form onSubmit={handleSubmit} className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
              ...
              <div className="px-5 pt-5 pb-4 border-b border-slate-100">
                <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-2.5">Module</p>
              ...
              <div className="px-5 py-4">
                ...
                <textarea
                  ...
                  disabled={demoState === 'evaluating'}
                />
              </div>

              {/* Quick prompts */}
              <div className="px-5 pb-4">
                <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-2">Try these prompts</p>
                  ...
                  {QUICK_PROMPTS[activeModule].map((qp, i) => (
                    ...
                    >
                      <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded flex-shrink-0 ${
                        qp.label === 'Safe' ? 'bg-emerald-100 text-emerald-700' :
                        qp.label === 'May flag' ? 'bg-amber-100 text-amber-700' :
                        'bg-red-100 text-red-700'
                      }`}>
                        {qp.label}
                      </span>
                      <span className="text-[12px] text-slate-600 group-hover:text-slate-800 transition-colors truncate">{qp.text}</span>
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div className="px-5 pb-5">
                <button
                  type="submit"
                  disabled={demoState === 'evaluating' || !promptText.trim()}
            ...
            <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
              ...
              {loadingSessions ? (
                ...
                <div className="px-5 py-8 text-center text-[13px] text-slate-400">No sessions yet</div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-slate-100">
                        <th className="text-left text-[11px] font-semibold text-slate-400 uppercase tracking-wide px-5 py-2.5">ID</th>
                        <th className="text-left text-[11px] font-semibold text-slate-400 uppercase tracking-wide px-3 py-2.5">Type</th>
                        <th className="text-left text-[11px] font-semibold text-slate-400 uppercase tracking-wide px-3 py-2.5">Status</th>
                        <th className="text-left text-[11px] font-semibold text-slate-400 uppercase tracking-wide px-3 py-2.5">Risk</th>
                        <th className="text-left text-[11px] font-semibold text-slate-400 uppercase tracking-wide px-3 py-2.5">When</th>
                      </tr>
                    </thead>
                      ...
                      {sessions.map(s => (
                        <tr
                          ...
                          onClick={() => {
                            ...
                            link.href = `/sessions`;
                            // Could navigate to session detail if it existed
                          }}
                        >
                          <td className="px-5 py-2.5">
                            <span className="font-mono text-[11px] text-slate-500">{s.id?.slice(0, 14)}</span>
                          </td>
                          <td className="px-3 py-2.5">
                            <span className="text-[12px] text-slate-600 capitalize">{s.session_type}</span>
                          </td>
                          <td className="px-3 py-2.5">
                            <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full border capitalize ${statusBadge(s.status)}`}>
                              {s.status}
                            </span>
                          </td>
                          <td className="px-3 py-2.5">
                            <span className={`text-[12px] font-bold ${
                              s.risk_score >= 80 ? 'text-red-600' :
                              s.risk_score >= 40 ? 'text-amber-600' :
                              'text-emerald-600'
                            }`}>{s.risk_score}</span>
                          </td>
                          <td className="px-3 py-2.5">
          ...
          <div className="flex flex-col gap-4">
            {demoState === 'idle' && (
              <div className="bg-white border border-slate-200 rounded-xl shadow-sm flex-1">
                <div className="h-full flex flex-col items-center justify-center px-6 py-16 text-center">
                  ...
                  <h3 className="text-[16px] font-bold text-slate-800 mb-2">Circuit Breaker Ready</h3>
                  <p className="text-[13px] text-slate-400 max-w-xs">
                    Submit a prompt to begin compliance evaluation. Each session is evaluated against active policy rules before any AI call is made.
                  </p>
                  <div className="mt-6 flex flex-col gap-2 w-full max-w-xs text-left">
                    {['Pre-flight rule evaluation', 'Energy score calculation', 'AI routing & generation'].map((step, i) => (
                      <div key={i} className="flex items-center gap-3 text-[12px] text-slate-400">
                        <span className="w-5 h-5 rounded-full border-2 border-slate-200 flex items-center justify-center text-[10px] font-bold text-slate-300">{i+1}</span>
                        {step}
                      </div>
                    ))}
                  </div>
            ...
            {demoState === 'evaluating' && (
              <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
                <div className="px-5 py-4 border-b border-slate-100 flex items-center gap-3">
                  ...
                  <span className="text-[14px] font-semibold text-slate-800">Evaluating Compliance...</span>
                </div>
                  ...
                  <div className="mt-6 flex items-center gap-3 text-[12px] text-slate-400">
                    <div className="flex-1 h-1 bg-slate-100 rounded-full overflow-hidden">
                      <div
                        ...
                        style={{ width: `${((evalStep + 1) / EVAL_STEPS.length) * 100}%` }}
                      />
                    </div>
                    <span>{evalStep + 1}/{EVAL_STEPS.length}</span>
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
              <div className="px-5 py-3.5 border-b border-slate-100">
                <h3 className="text-[13px] font-semibold text-slate-700">How the Circuit Breaker Works</h3>
              </div>
              <div className="px-5 py-4">
                <div className="grid grid-cols-3 gap-3">
                  {[
                    { icon: '🟢', label: 'Safe', desc: 'No rules triggered. AI responds normally.' },
                    { icon: '🟡', label: 'Frozen', desc: 'Policy boundary hit. Human review required.' },
                    { icon: '🔴', label: 'Blocked', desc: 'Critical violation. No AI call made.' },
                  ].map(item => (
